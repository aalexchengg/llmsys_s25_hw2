...
__global__ void MatrixMultiplyKernel(
   ...
   */
    ...
    int out_row = blockIdx.x * blockDim.x + tx; // this gives us the output row of dimension M
    int out_col = blockIdx.y * blockDim.y + ty; // this gives us the output col of dimension P
    int out_pos = batch * out_strides[0] + out_row * out_strides[1] + out_col; // this indexes into the compact array


    float pvalue = 0;
    int width = a_shape[2]; // n
    for (int i = 0; i < ceil(width/(float)TILE); i++) {
      ...
      if (in_index[0] < a_shape[0] && in_index[1] < a_shape[1] && in_index[2] < a_shape[2]) {
      ...
      if (in_index[0] < b_shape[0] && in_index[1] < b_shape[1] && in_index[2] < b_shape[2]) {
      ...
      for (int j = 0; j < TILE; j++) {        
        pvalue += a_shared[tx][j] * b_shared[j][ty];
      }
      __syncthreads();
    }
